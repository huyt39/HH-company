...
/** Danh sách ảnh phụ — thêm, đổi, xoá và sắp xếp lại. */
export default function GalleryPicker({ value = [], onChange, label }) {
      ...
      {items.map((item, index) => (
        <div className="gallery-picker__row" key={`${item.url}-${index}`}>
          ...
          <input
            ...
            value={item.alt ?? ''}
            onChange={(e) =>
              onChange(items.map((it, i) => (i === index ? { ...it, alt: e.target.value } : it)))
            }
          />
          <div className="gallery-picker__buttons">
            ...
            <button type="button" className="is-danger" onClick={() => onChange(items.filter((_, i) => i !== index))} aria-label="Xoá">✕</button>
          </div>
        </div>
      ))}
