...
/**
 * Ô chọn ảnh: tải ảnh mới hoặc chọn lại ảnh đã có trong thư viện.
 *
 * @param {{value: object|null, onChange: (media: object|null) => void, label: string}} props
 *   `value` có dạng { url, alt } — khớp schema Media của backend.
 */
export default function ImagePicker({ value, onChange, label, hint }) {
  ...
  useEffect(() => {
    if (!open) return
    adminApi.listImages().then(setLibrary).catch((err) => setError(err.message))
  }, [open])
    ...
    <div className="image-picker">
      ...
      {open && (
        <div className="image-modal" role="dialog" aria-modal="true">
          <button type="button" className="image-modal__backdrop" aria-label="Đóng" onClick={() => setOpen(false)} />
          <div className="image-modal__panel">
            ...
            <div className="image-modal__body">
              <label className="image-upload">
                <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" onChange={handleFile} disabled={busy} />
                <span>
                  {busy
