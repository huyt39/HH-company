"""Dữ liệu tĩnh trích từ hồ sơ pháp lý và hồ sơ năng lực của công ty.

Nguồn:
  - Giấy chứng nhận ĐKKD số 0106346833, thay đổi lần 8 ngày 16/12/2025.
  - Hồ sơ năng lực HSNL-HH 2026.06.23 R5 (Hà Nội, 2026).

Đây là nơi tạm giữ nội dung cho tới khi có database. Khi gắn DB, chuyển các
hằng số dưới đây thành bản ghi và cập nhật `app/services/store.py`.
"""
...
# --------------------------------------------------------------------------- #
# Lĩnh vực hoạt động (mục 6, hồ sơ năng lực)
# --------------------------------------------------------------------------- #
...
BUSINESS_FIELDS = [
    BusinessField(id=str(i), slug=slug, name=name, description=desc, icon=icon)
    for i, (slug, name, desc, icon) in enumerate(_FIELDS, start=1)
]

# --------------------------------------------------------------------------- #
# Nhóm sản phẩm cung cấp (phần "Các sản phẩm cung cấp", hồ sơ năng lực)
# --------------------------------------------------------------------------- #
...
PRODUCTS = [Product(id=str(i), **p) for i, p in enumerate(_PRODUCTS, start=1)]

# --------------------------------------------------------------------------- #
# Danh sách dự án đã cung cấp và thi công
# Nguồn: bảng "CÁC DỰ ÁN ĐÃ CUNG CẤP VÀ THI CÔNG" + các thư chấp thuận 2025–2026.
# --------------------------------------------------------------------------- #

# (slug, tên, năm, khách hàng, địa điểm, phạm vi cung cấp, trạng thái)
...
# Bối cảnh dự án tổng hợp từ nguồn tin công khai (báo chí, cổng thông tin cơ quan
# nhà nước, website chủ đầu tư) — tách riêng khỏi phạm vi cung cấp của Hòa Hoàng.
# Định dạng: slug -> (nội dung, url nguồn)
...
PROJECTS = [
    ...
    for i, (slug, name, year, client, location, scope, status) in enumerate(_PROJECTS, start=1)
]

# --------------------------------------------------------------------------- #
# Số liệu tài chính 3 năm gần nhất (báo cáo B01a/B02 - DNN, đơn vị: VNĐ)
# --------------------------------------------------------------------------- #
...
PARTNERS = [
    Partner(name="Tổng Công ty Đầu tư phát triển đường cao tốc Việt Nam (VEC)", country="Việt Nam", role="customer"),
    Partner(name="Tổng Công ty Xây dựng Trường Sơn", country="Việt Nam", role="customer"),
    Partner(name="CTCP – Tổng Công ty Cơ khí Xây dựng Thăng Long (MECO)", country="Việt Nam", role="customer"),
    Partner(name="Công ty TNHH Xây dựng và Thương mại Trung Chính", country="Việt Nam", role="customer"),
    Partner(name="Công ty TNHH Phát triển Phú Mỹ Hưng", country="Việt Nam", role="customer"),
    Partner(name="CTCP Tập đoàn Đạt Phương", country="Việt Nam", role="customer"),
    Partner(name="Tổng Công ty Xây dựng số 1 – CTCP", country="Việt Nam", role="customer"),
    Partner(name="Tổng Công ty Xây dựng công trình giao thông 8 – CTCP", country="Việt Nam", role="customer"),
    Partner(name="CTCP Tập đoàn Thuận An – TAG", country="Việt Nam", role="customer"),
    Partner(name="CTCP Xây dựng công trình 525", country="Việt Nam", role="customer"),
    Partner(name="CTCP ĐTXD Thương mại Trường Thành", country="Việt Nam", role="customer"),
    Partner(name="Công ty Cổ phần Cầu đường Long Biên", country="Việt Nam", role="customer"),
    Partner(name="Công ty Cổ phần SBTECH", country="Việt Nam", role="customer"),
    Partner(name="Công ty Cổ phần Cơ khí Cầu đường Hà Ninh", country="Việt Nam", role="customer"),
    Partner(name="Kumho Engineering & Construction", country="Hàn Quốc", role="customer"),
    Partner(name="SHINKO", country="Nhật Bản", role="manufacturer"),
    Partner(name="APS", country="Nhật Bản", role="manufacturer"),
    Partner(name="Hirun", country="Italy", role="manufacturer"),
    Partner(name="Mageba (Shanghai) Bridge Products", country="Trung Quốc", role="manufacturer"),
    Partner(name="Liuzhou OVM Machinery", country="Trung Quốc", role="manufacturer"),
    Partner(name="CSSC Sunrui Luoyang Special Equipment (Song Thụy Lạc Dương Trung Thuyền)", country="Trung Quốc", role="manufacturer"),
    Partner(name="Công ty cáp Quế Cầu Liễu Châu", country="Trung Quốc", role="manufacturer"),
    Partner(name="Công ty TNHH Cơ khí DƯL HM Liễu Châu", country="Trung Quốc", role="manufacturer"),
    Partner(name="Công ty phát triển công nghệ giao thông Vạn Cầu Trùng Khánh", country="Trung Quốc", role="manufacturer"),
]
